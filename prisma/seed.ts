import { PrismaClient, TicketStatus, BoardType } from "@prisma/client";
...
async function users() {
  ...
  const board2 = await prisma.board.upsert({
    ...
    create: {
      ...
      endDate: new Date(
        new Date().getTime() + 17 * 24 * 60 * 60 * 1000
      ).toISOString(),
    },
  });
